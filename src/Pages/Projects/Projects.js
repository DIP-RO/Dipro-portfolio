  ...
  const { data: projects, isLoading } = useQuery({
    ...
    queryFn: async () => {
      const res = await fetch(
        "https://its-dipro-portfolio-server-main.vercel.app/projects"
      );
  ...
  return (
    <div className="p-4">
      <div className="relative z-20 my-2 md:my-10">
        ...
        <div className="grid grid-cols-1 gap-0 gap-y-4 md:gap-y-0 md:gap-10 justify-items-center ml-0 md:ml-6 py-10 md:grid-cols-2 lg:grid-cols-1 px-2 md:mx-10">
          ...
          <div className="grid gap-y-4  grid-cols-1">
            {isLoading ? (
              ...
              <>
                {/* {
                                        projects.map(project => <SmallProjectCard
                                            key={project._id}
                                            project={project}
                                        ></SmallProjectCard>)
                                    } */}
        ...
        <div className="grid md:mx-14 grid-cols-1 md:grid-cols-3 lg:grid-cols-1 gap-5">
          {isLoading ? (
            <>
              <div className="flex flex-col  rounded shadow-md  w-96  animate-pulse h-72">
                <div className="h-48 rounded-t dark:bg-gray-700"></div>
      ...
      <div className="absolute top-32  left-0 z-10 white_bg rounded-full w-[40%] h-[40%]"></div>
      <ScrollRestoration />
    </div>
  );
};
